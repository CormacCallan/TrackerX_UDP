...
$query3 = "SELECT * FROM subject WHERE lecture_id=:lecture_id";
$statement3 = $db->prepare($query3);
$statement3->bindValue(':lecture_id', $_SESSION['lecturer_session']);
$statement3->execute();
$subjects = $statement3->fetchAll();
    ...
    <div class="container">
        ...
        <?php foreach ($subjects as $subject) : ?>
            ...
            <div class="upcoming timetable" id="Mon">
                <p class="up-title"><?php              
                echo $subject['day']; ?> </p>
                <ul class="up-plan">
                    <li><span class="roundhead"><i class="material-icons"></i></span><?php echo $subject['subject_name']; ?><span class="secondo">Room : <?php
            $query4 = "SELECT * FROM rooms WHERE room_id =:room_id";
            $statement4 = $db->prepare($query4);
            $statement4->bindValue(':room_id', $subject['room_id']);
            $statement4->execute();
            $row3 = $statement4->fetch();
            echo $row3['room_number'];
            ?></span><p class="up-date"><?php echo $subject['start_time'] . " to " . $subject['end_time']; ?></p></li>
                </ul>
            </div>
